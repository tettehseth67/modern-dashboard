...
import { ShieldLock, Mail, Lock, AlertCircle } from "lucide-react";
...
export default function LoginPage() {
    ...
    // Pre-emptive check: If user already has a running session token, clear the login gate
    useEffect(() => {
            ...
            <div className="w-full max-w-md bg-slate-800 border border-slate-700 rounded-2xl p-8 shadow-2xl space-y-6">
                ...
                <form onSubmit={handleLogin} className="space-y-4">
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-semibold transition-colors disabled:opacity-50 cursor-pointer flex items-center justify-center gap-2 mt-2 shadow-lg"
                    >
                        {isLoading ? "Authenticating Session..." : "Sign In to Workspace"}
                    </button>
                </form>
            </div>
        </div>
